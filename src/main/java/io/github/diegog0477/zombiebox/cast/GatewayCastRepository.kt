...
class GatewayCastRepository(private val prefs: SharedPreferences) : CastRepository {
    val api=GatewayApi().apply { configure(prefs.getString("gateway","")!!,prefs.getString("device","")!!,prefs.getString("token","")!!) }
    override fun pair(address: String,code: String) {
        val base=address.trim().trimEnd('/');val uri=URI(base)
    ...
    override fun receivers():List<Receiver> {
        val result=api.request("GET","/v1/cast/receivers");check(result.optBoolean("relayAvailable"))
        val items=result.getJSONArray("receivers")
        return (0 until items.length()).map {items.getJSONObject(it)}.filter {it.optBoolean("online")}.map {Receiver(it.getString("deviceId"),it.optString("name").ifEmpty {it.getString("deviceId")})}
    }
    override fun create(receiver: String):CastGrant {
        val result=api.request("POST","/v1/cast",JSONObject().put("receiverId",receiver))
        return CastGrant(result.getString("castId"),URI(api.base).host,result.getInt("rtspPort"),result.getString("publishPath"),result.getString("publishUser"),result.getString("publishToken"))
    }
    override fun stop(id: String) { api.request("DELETE","/v1/cast/$id") }
}
